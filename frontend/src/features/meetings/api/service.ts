...
export const fetchMeetingUploads = (meetingId?: number | null) => {
	...
	return request<MeetingUpload[]>(
		`/meetings/uploads${suffix}`,
		undefined,
		true,
		meetingUploadsSchema,
	)
...
export const addMeetingUploads = (files: string[], meetingId?: number | null) =>
	request<MeetingUpload[]>(
		'/meetings/uploads',
		{
			method: 'POST',
			body: addMeetingUploadsPayloadSchema.parse({
