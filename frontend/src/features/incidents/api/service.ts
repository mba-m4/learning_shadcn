...
import {
	...
	incidentSchema,
	usersResponseSchema,
} from '@/shared/api/schemas/incidents'
...
export const updateIncidentAssignment = (
	incidentId: number,
...
export const removeIncidentLabel = (incidentId: number, label: string) =>
	request<Incident>(
		`/incidents/${incidentId}/labels/${encodeURIComponent(label)}`,
		{
			method: 'DELETE',
		},
		true,
...
export const addIncidentComment = (incidentId: number, content: string) =>
	request<IncidentComment>(
		...
		true,
		incidentCommentsSchema.element,
	)

export const updateIncidentComment = (
	incidentId: number,
	commentId: number,
	content: string,
) =>
