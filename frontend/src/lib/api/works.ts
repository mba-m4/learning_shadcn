...
export const createGroup = (name: string) =>
  request<WorkGroup>('/works/groups', {
    method: 'POST',
    body: { name },
  }, true, workGroupsResponseSchema.element)
...
export const addWorkItem = (workId: number, payload: { name: string; description: string }) => {
  const parsedPayload = createWorkItemPayloadSchema.parse(payload)

  return request<WorkItem>(`/works/${workId}/items`, {
    method: 'POST',
    body: parsedPayload,
  }, true, workItemSchema)
}
...
export const generateRisk = (workItemId: number) =>
  request<RiskAssessment>(`/works/items/${workItemId}/risks/generate`, {
    method: 'POST',
